...
- Accumulating validation, source-scanning, pass, and writer domains: `prelude.validation`,
  `prelude.span`, `prelude.scan`, `prelude.pass`, and `prelude.writer`.
...
directly too, including `prelude.core`, `prelude.layout`, `prelude.fixed`, `prelude.fixed_build`,
`prelude.range`, `prelude.scalar`, `prelude.order`, `prelude.effect`, `prelude.optic`,
`prelude.derive`, and `prelude.geometry2d`.
...
| `prelude.option`     | `some`, `none`, `guard`, plus `Option::is_some`, `Option::is_none`, `Option::unwrap_or`, `Option::map`, `Option::pure`, `Option::apply`, `Option::bind`, `Option::append`, `Option::empty`, `Option::or_else`, `Option::or_else_with`, and `Option::unwrap_or_else_with`. |
...
| `prelude.validation` | `Validation(error, value)` with `valid`, `invalid`, validity predicates, `unwrap_or`, `map`, `map_error`, `pure`, accumulating `apply` through `Semigroup(error)`, and conversion to/from `Result`.                |
...
functions evaluate their requirements with local `@assert(Contract(t));` calls, so callers pass only
the runtime values:
...
`prelude.writer` exposes `Writer(Log, A)` for append-only logs such as diagnostics or emitted IR.
It provides `tell`, `listen`, `run`, `value`, `log`, and functor/applicative/monad members. `pure`
requires `Monoid(Log)`; sequencing combines logs with `Semigroup(Log)`.

`prelude.pass` exposes `Pass(Env, State, Log, A)`, a reader/state/writer carrier for analysis or
lowering passes. It has `ask`, `asks`, `get`, `gets`, `put`, `modify`, `tell`, `emit`, `run`,
`eval`, `exec`, `log`, and functor/applicative/monad members. It is intended for pass contexts where
threading environment, mutable analysis state, and diagnostics or output together would otherwise
inflate function signatures.

`prelude.scan` exposes `Scan(Input, A)`, a reader+cursor carrier for parser/scanner style code. It
has `ask`, `cursor`, `set_cursor`, `advance`, `consume`, `with_span`, `run`, `run_from_start`, and
functor/applicative/monad members. `consume` advances a `prelude.span.Cursor` and returns the
consumed `Span`; `with_span` wraps a scan result with the span consumed by that scan.

`prelude.effect` builds transparent reader/state effect rows with `Eff`, `EffContext`, `EffResult`,
`ReaderValue`, `StateValue`, `ask`, `get`, `put`, `modify`, `run_state`, `run_reader`, and
`run_pure`. The effect row contracts are checked through ordinary type functions.

`prelude.optic` provides function-based lens helpers: `view`, `set`, `over`, `preview`, `view_or`,
`set_optional`, and `over_optional`.

`prelude.span` provides `Span` and `Cursor` domains for source-like scans and diagnostics. `Span`
has constructors plus length, emptiness, containment, intersection, merge, and shift helpers.
`Cursor` tracks an index with a limit and exposes start, advance, remaining, end checks, and span
construction between cursor positions.
...
`vertex2d`, `quad2d_rect`, `quad2d_translate`, `emit_quad2d`, `emit_rect2d`, batch construction, and
append helpers.
